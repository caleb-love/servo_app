...
async function initMap() {
  ...
  if ("geolocation" in navigator) {
    navigator.geolocation.getCurrentPosition(position => {
      ...
      function updateLocationInfo() {
        ...
        geocoder.geocode({ location: { lat, lng } }, (results, status) => {
          if (status === "OK") {
            ...
            locElement.textContent = "geocoder error"
          }
        })
      }
      ...
      map.addListener("idle", () => {
        ...
        fetchStationsInBound(southLat, northLat, westLng, eastLng)
          .then(res => {
            // setMapOnAll(null)
            res.forEach(station => {
              if (!displayedStations.some(s => s.id === station.id)) {
                const icon = {
                  url: station.logo,
          ...
          .then(res => {
            displayedStations.forEach(station => {
              if (!map.getBounds().contains(station.marker.getPosition())) {
      ...
      map.controls[google.maps.ControlPosition.LEFT_TOP].push(backButton)
...
function getRandomStation() {
  fetchRandom().then(
    ({ name, owner, latitude, longitude, address, suburb, state, logo }) => {
      let spotlightMarker
...
const sections = document.querySelectorAll(
  ".clock, .stats-section, .about, .nearest-ps"
)
...
const partyModeToggle = document.querySelector("#party-mode-toggle")
...
partyModeToggle.addEventListener("click", () => {
  if (partyModeInterval) {
    ...
    partyModeInterval = setInterval(() => {
      ...
      mapElement.classList.toggle("party-mode-map")
      map.setOptions({
        styles: [
          {
            elementType: "labels.text.stroke",
