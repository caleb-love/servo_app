...
class Station {
	...
	static async random_station() {
		const countRes = await db.query('SELECT COUNT(*) FROM stations')
		const count = countRes.rows[0].count
		const offset = Math.floor(Math.random() * count)
		const sql = 'SELECT * FROM stations LIMIT 1 OFFSET $1'
		const res = await db.query(sql, [offset])
		return res.rows[0]
	}
	...
	static in_bounds(southLat, northLat, westLng, eastLng) {
		...
		return db
			.query(sql, [southLat, northLat, westLng, eastLng])
			.then((res) => res.rows)
	}

	static stats() {
		const sql1 =
			'select owner, count(owner) from stations group by owner having count (owner) > 1 order by count desc;'
		const sql2 =
			'select count(*) as total_stations, count(distinct owner) as total_owners from stations;'

		let owners = db.query(sql1)
		let total_stations = db.query(sql2)
		return Promise.all([owners, total_stations]).then((dbRes) => {
	...
	static stations_nearby(lat, lng, radius) {
        ...
        `
		return db
			.query(query, [lat, lng, radius * 1000])
			.then((res) => res.rows)
	}
}
